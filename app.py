    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user_check and security.check_password_hash(user_check['password'], password):
                ...
                flash(f"Welcome, {html_encode(user_check['username'])}! Your email is {html_encode(user_check['email'])}.")
                conn.close()
...
def register():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            result = conn.execute(check_query, (username, email))
            ...
            conn.execute(insert_query, (username, email, hashed_password))
            conn.commit()
            conn.close()
